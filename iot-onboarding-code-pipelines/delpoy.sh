...
then
    ...
    #This section allows user to deploy only 1 stack or all stacks
        ...
        #cdk deploy  -c envName=$env --format=json --require-approval never --parameters contactEmail="support@synectiks.com" --parameters quickSightAdminUserName="admin/rollatgr-Isengard" --parameters sourceTemplateArn="arn:aws:quicksight:eu-central-1:660526416360:template/iotOnboardingRigadoQuicksightPublicTemplatedev" --parameters rootMqttTopic="data/#"
        rc=$?
    if [ $rc -ne 0 ]; then
